...
@st.cache_data(ttl=3600)
def get_monthly_liquidity_data(year, month, num_months=36):
    try:
        conn = get_connection(
            host='data-query.hec.co.kr',
            port=443,
...
def get_additional_data(year, month):
    try:
        conn = get_connection(
            host='data-query.hec.co.kr',
            port=443,
        ...
        SELECT 
            ...
            SUM(gc) AS total_gc
        FROM s_accounting_ledger
        WHERE posting_date >= DATE '{year}-{month:02d}-01'
          AND posting_date < DATE_ADD('month', 1, DATE '{year}-{month:02d}-01')
          AND ((CAST(acc_num AS BIGINT) >= 11010110 AND CAST(acc_num AS BIGINT) <= 11110190)
           OR (CAST(acc_num AS BIGINT) >= 17000000 AND CAST(acc_num AS BIGINT) <= 17999999))
        GROUP BY acc_num, acc_name, profit_center, profit_center_name
        ORDER BY acc_num, profit_center
        """
        ...
        st.error(f"추가 데이터 로딩 중 오류 발생: {str(e)}")
        return pd.DataFrame()
...
def analyze_time_series(df, end_date):
    ...
    for acc_num in df['acc_num'].unique():
        ...
        if len(series) >= 2:  # 최소 2개 이상의 데이터 포인트가 있는 경우
    ...
    if st.session_state.menu == "계정추세 분석":
        ...
        if st.button("유동성 분석 실행"):
            try:
                ...
                additional_df = get_additional_data(year, month) 
                ...
                st.subheader("월변동 기여도 분석")
                st.dataframe(additional_df)

                # Profit Center Name별 Total GC 막대 그래프
                additional_summary = additional_df.groupby('profit_center_name')['total_gc'].sum().sort_values(ascending=False).reset_index()

                fig = go.Figure(data=[go.Bar(
                    x=additional_summary['total_gc'],
                    y=additional_summary['profit_center_name'],
                    orientation='h',
                    hovertemplate='Profit Center: %{y}<br>금액: %{x:,.0f}'
                )])
                fig.update_layout(
                    title='Profit Center Name별 Total GC (내림차순)',
                    xaxis_title='Total GC',
                    yaxis_title='Profit Center Name',
                    height=max(600, len(additional_summary) * 20),  # 그래프 높이 동적 조정
                    hoverlabel=dict(bgcolor="white", font_size=12)
                )
                st.plotly_chart(fig, use_container_width=True)                
                ...
                st.error(f"유동성 분석 중 오류가 발생했습니다: {str(e)}")
                st.exception(e)
